...
open System.Linq
open System.Text
open VSCodeHtmlCopy.Arguments
open VSCodeHtmlCopy.Clipboard

[<EntryPoint>]
let main argv =
    let errorHandler = ProcessExiter(colorizer = function ErrorCode.HelpText -> None | _ -> Some ConsoleColor.Red)
    let parser = ArgumentParser.Create<Arguments>(programName = "vscode-html-copy", errorHandler = errorHandler)
    ...
    let fragment =
        ...
        | Some className ->
            ...
            nodes |> Seq.collect (getAllElements(fun (e: IElement) -> e.TagName = TagNames.Span && String.IsNullOrWhiteSpace(e.TextContent)))
